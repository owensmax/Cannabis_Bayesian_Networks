...
#ADD PERSONALITY MEAUSURES
SURPS_parent_bsl <-read.delim("/home/max/Documents/Bayes/SURPS_PARENT_BSL.tsv")
SURPS_child_bsl <-read.delim("/home/max/Documents/Bayes/SURPS_CHILD_BSL.tsv")
SURPS_child_fu2 <-read.delim("/home/max/Documents/Bayes/SURPS_CHILD_FU2.tsv")
NEO_parent_bsl <-read.delim("/home/max/Documents/Bayes/NEO_PARENT_BSL.tsv")
NEO_child_bsl <-read.delim("/home/max/Documents/Bayes/NEO_CHILD_BSL.tsv")
NEO_child_fu2 <-read.delim("/home/max/Documents/Bayes/NEO_CHILD_FU2.tsv")

personality <- inner_join(SURPS_child_fu2,SURPS_child_bsl)
#personality <- inner_join(personality,SURPS_child_fu2)
#personality <- inner_join(personality,NEO_parent_bsl)
#personality <- inner_join(personality,NEO_child_bsl)
#personality <- inner_join(personality,NEO_child_fu2)
names(personality)[1] <- "Subject"
...
candat3 <- inner_join(candat2,personality)

#test for most related personality variable to cannabis
cor.test(candat3$Life_FU2,candat3$surps_c_sensation_seeking_sum_fu2)
cor.test(candat3$Life_FU2,candat3$surps_c_impulsivity_average_fu2)
cor.test(candat3$Life_FU2,candat3$surps_c_sensation_seeking_sum_bsl)
cor.test(candat3$Life_FU2,candat3$surps_c_impulsivity_average_bsl)
cor.test(candat3$surps_c_sensation_seeking_sum_fu2,candat3$surps_c_impulsivity_average_fu2)
cor.test(candat3$surps_c_sensation_seeking_sum_bsl,candat3$surps_c_impulsivity_average_bsl)

#make reduced dataset
imagen_data=Reduce(function(x,y) merge(x = x, y = y, by = "Subject"), 
       list(ROIs, candat3[c('Subject','Site','Sex','Handedness','Baseline_Age','Life_BSL','Mod_to_Heavy_at_BSL','Site_FU2','Sex_FU2','Hand_FU2','Age_FU2','Life_FU2','Mod_to_Heavy_at_FU2','Life_Nicotine_BSL','Life_Nicotine_FU2','AUDIT_C_FU2','AUDIT_C_BSL','S5CANNnew_FU2','S5CUDnew_FU2','C1_FU2','C2_FU2','C3_FU2','C4_FU2',"SES","baseline_PDS","ADHD_bl","change_ADHD","CTQ_total","surps_c_sensation_seeking_sum_fu2", "surps_c_impulsivity_average_fu2","surps_c_sensation_seeking_sum_bsl","surps_c_impulsivity_average_bsl")]))
...
imagen_data$Sensation_Seeking_change<-imagen_data$surps_c_sensation_seeking_sum_fu2-imagen_data$surps_c_sensation_seeking_sum_bsl
imagen_data$Impulsivity_change<-imagen_data$surps_c_impulsivity_average_fu2-imagen_data$surps_c_impulsivity_average_bsl
...
#SS BL
model1<-lmer(surps_c_sensation_seeking_sum_bsl~(1|Site) + (1|Sex), data=imagen_data)
Sensation_Seeking_bl<- as.matrix(residuals(model1))

#SS FU
model1<-lmer(Sensation_Seeking_change~(1|Site) + (1|Sex), data=imagen_data)
Sensation_Seeking_change<- as.matrix(residuals(model1))

#Impulsivity BL
model1<-lmer(surps_c_impulsivity_average_bsl~(1|Site) + (1|Sex), data=imagen_data)
Impulsivity_bl<- as.matrix(residuals(model1))

#Impulsivity FU
model1<-lmer(Impulsivity_change~(1|Site) + (1|Sex), data=imagen_data)
Impulsivity_change<- as.matrix(residuals(model1))
...
write.csv(correlation_matrix(imagen_data[c('Handedness','Baseline_Age','Life_Nicotine_BSL','AUDIT_C_BSL','S5CANNnew_FU2','Life_change','AUDIT_change','Life_Nicotine_change','brain.pca.bl','brain.pca.change',"SES","baseline_PDS","ADHD_bl","change_ADHD","CTQ_total","surps_c_impulsivity_average_bsl", "Impulsivity_change", "surps_c_sensation_seeking_sum_bsl", "Sensation_Seeking_change")], 
           type = "pearson",
           digits = 2, 
           decimal.mark = ".",
           use = "lower", 
...
light_users <- filter(can_users, Life_change < 4)
...
can_user_raw_change <- (mean(bilateral_dmpfc_BSL)-mean(bilateral_dmpfc_FUP))
can_user_perc_change <- (mean(bilateral_dmpfc_BSL)-mean(bilateral_dmpfc_FUP))/(mean(bilateral_dmpfc_BSL))
...
non_user_raw_change <- (mean(bilateral_dmpfc_BSL)-mean(bilateral_dmpfc_FUP))
non_user_perc_change <- (mean(bilateral_dmpfc_BSL)-mean(bilateral_dmpfc_FUP))/(mean(bilateral_dmpfc_BSL))
...
heavy_user_raw_change <- (mean(bilateral_dmpfc_BSL)-mean(bilateral_dmpfc_FUP))
heavy_user_perc_change <- (mean(bilateral_dmpfc_BSL)-mean(bilateral_dmpfc_FUP))/(mean(bilateral_dmpfc_BSL))

bilateral_dmpfc_FUP <- rowMeans(light_users[c('right_dmPFC_FUP', 'left_dmPFC_FUP')])
bilateral_dmpfc_BSL <- rowMeans(light_users[c('right_dmPFC_BSL', 'left_dmPFC_BSL')])
light_users_raw_change <- (mean(bilateral_dmpfc_BSL)-mean(bilateral_dmpfc_FUP))
light_users_perc_change <- (mean(bilateral_dmpfc_BSL)-mean(bilateral_dmpfc_FUP))/(mean(bilateral_dmpfc_BSL))

((can_user_perc_change - non_user_perc_change)/((can_user_perc_change + non_user_perc_change)/2))
((heavy_user_perc_change - non_user_perc_change)/((heavy_user_perc_change + non_user_perc_change)/2))

can_naive <- filter(imagen_data,Life_change==0)
can_use <- filter(imagen_data,Life_change>1)

#new stuff for RR2
imagen_data$average_dmpfc_bsl <- rowMeans(imagen_data[c('left_dmPFC_BSL','right_dmPFC_BSL')])
imagen_data$average_dmpfc_fu2 <- rowMeans(imagen_data[c('left_dmPFC_FUP','right_dmPFC_FUP')])
imagen_dmpfc_percent_change <- (imagen_data$average_dmpfc_bsl - imagen_data$average_dmpfc_fu2)/imagen_data$average_dmpfc_bsl
mean(imagen_dmpfc_percent_change)
imagen_dmpfc_change <- (imagen_data$average_dmpfc_bsl - imagen_data$average_dmpfc_fu2)
mean(imagen_dmpfc_change)
...
data.regressed <- cbind(Baseline_dPFC_Thickness,Change_in_dPFC_Thickness,Cannabis_Use_by_Age_19,Baseline_Tobacco_Use,Change_in_Tobacco_Use,Baseline_Alcohol_Use,Change_in_Alcohol_Use,Handedness,Baseline_Age,S5CANNnew_FU2,SES,baseline_PDS,ADHD_bl,change_ADHD,CTQ_total, Sensation_Seeking_bl, Sensation_Seeking_change)## assembles the residuals
...
blacklist <- read.delim("/home/max/Documents/Bayes/blacklist_doc_V3.csv")

## generate the blacklist that includes cortical thickness to cannabis use on blacklist
blacklist_Brain2Cannabis <- read.delim("/home/max/Documents/Bayes/blacklist_doc_cannabis2brainblocked.csv")

##  generate the blacklist that includes cannabis use to cortical thickness on blacklist
blacklist_Cannabis2Brain <- read.delim("/home/max/Documents/Bayes/blacklist_doc_brain2cannabisblocked.csv")
...
boot[(boot$strength > 0.6) & (boot$direction >= 0.5), ]
...
boot_c2b[(boot_c2b$strength > 0.6) & (boot_c2b$direction >= 0.5), ]
...
boot_b2c[(boot_b2c$strength > 0.6) & (boot_b2c$direction >= 0.5), ]
...
boot[(boot$strength > 0.6) & (boot$direction >= 0.7), ]
